...
function afficherCotisations(liste) {
  ...
  afficherStatsCotisationsSpeciales(liste);
...
function afficherStatsCotisationsSpeciales(liste) {
  const montantTotal = liste.reduce((sum, c) => sum + (Number(c.montant) || 0), 0);
  document.getElementById("statCsTotal").textContent = liste.length;
  document.getElementById("statCsPayees").textContent =
    liste.filter((c) => c.statut === "payee").length;
  document.getElementById("statCsEnAttente").textContent =
    liste.filter((c) => c.statut === "en_attente").length;
  document.getElementById("statCsMontant").textContent = formatMontant(montantTotal);
}
