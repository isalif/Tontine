...
class AuthController
{
    ...
    public static function register(): void
    {
        try {
            ...
            $membreId = null;
            if ($username !== null) {
                $membre = Membre::findByUsername($username);
                // Message volontairement générique : ne pas révéler si le nom d'utilisateur
                // existe ou s'il est simplement déjà relié à un autre compte.
                if (!$membre || Utilisateur::findByMembreId($membre['id'])) {
                    send_error("Nom d'utilisateur invalide ou déjà utilisé", 400);
                }
